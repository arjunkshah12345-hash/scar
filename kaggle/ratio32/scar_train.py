"""Kaggle CPU driver for Study 2B, training context 32."""
import json
import os
import shutil
import subprocess
...
OUT = WORK / "study2_results" / "ratio32"
...
(OUT / "manifest.json").write_text(json.dumps({
    ...
    "experiment_family": "v3B_recall_train32", "git_commit": commit,
    "models": MODELS, "seeds": list(SEEDS), "train_ops": 32,
    "eval_lengths": [int(x) for x in EVAL_LENGTHS.split(",")],
}, indent=2))

for model in MODELS:
    for seed in SEEDS:
        experiment_id = f"v3B_recall_train32_{model}_seed{seed}"
        ...
        cmd = [sys.executable, "bench.py", "--model", model, "--task", "recall",
               ...
               "--train_ops", "32", "--eval_lengths", EVAL_LENGTHS,
...
shutil.copytree(OUT, "/kaggle/working/study2-results/ratio32", dirs_exist_ok=True)
shutil.copytree(logs, "/kaggle/working/study2-logs/ratio32", dirs_exist_ok=True)
